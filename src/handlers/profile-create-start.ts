...
import { canonicalCity } from "../cities.js";
import { adminChatId, inlineButton, inlineKeyboard, registerMainMenuItem } from "../toolkit/index.js";
import { activeRegistrationBlock, blockMessage, enforceViolation, inspectProfileText, recordPolicyAudit, registrationState, wordCount } from "../content-policy.js";
...
composer.on("message:text", async (ctx, next) => {
  ...
  if (ctx.session.step === "city") { const city = canonicalCity(text); if (!city) { const decision = { kind: "suggestion" as const, message: "📍 Такого города не найдено. Пожалуйста, укажите существующий город", confidenceScore: 1, submittedText: text }; await recordPolicyAudit(ctx, "city-rejected", decision); await ctx.reply(decision.message, { reply_markup: force("Введите город") }); return; } d.city = city; begin(ctx, "marital"); await ctx.reply("Каков ваш семейный статус?", { reply_markup: choose([[inlineButton("Не был(а) в браке", "profile:marital:single"), inlineButton("В отношениях", "profile:marital:relationship")], [inlineButton("Разведён(а)", "profile:marital:divorced"), inlineButton("Вдовец или вдова", "profile:marital:widowed")]]) }); return; }
  ...
  if (["profession", "about", "purpose"].includes(ctx.session.step ?? "")) { const step = ctx.session.step; const maxLength = step === "about" ? 10_000 : 500; if (text.length < 2 || text.length > maxLength) { await ctx.reply(step === "about" ? "Текст слишком длинный. Укажите от 50 до 1000 слов." : "Ответ должен быть от 2 до 500 символов. Попробуйте ещё раз.", { reply_markup: force("Введите ответ") }); return; } if (step === "profession") d.profession = text; if (step === "about") { const words = wordCount(text); if (words < 50 || words > 1000) { const decision = { kind: "suggestion" as const, message: "Ваш текст должен содержать от 50 до 1000 слов. Пожалуйста, отредактируйте и отправьте снова.", confidenceScore: 1, submittedText: text }; await recordPolicyAudit(ctx, "bio-length-rejected", decision); await ctx.reply(decision.message, { reply_markup: force("Расскажите о себе") }); return; } const decision = inspectProfileText({ bio: text }); await recordPolicyAudit(ctx, decision.kind === "allowed" ? "bio-accepted" : "bio-review", decision); if (decision.kind === "suggestion") { await ctx.reply(decision.message!, { reply_markup: force("Расскажите о себе") }); return; } if (decision.kind === "explicit") { const event = await enforceViolation(ctx, decision); ctx.session.step = "idle"; ctx.session.draft = undefined; await ctx.reply(blockMessage(event!), { reply_markup: menu }); return; } d.bio = text; } if (step === "purpose") d.purpose = text; const nextStep = step === "profession" ? "height" : step === "about" ? "purpose" : "telegram"; begin(ctx, nextStep); if (nextStep === "height") await ctx.reply("Какой у вас рост в сантиметрах?", { reply_markup: force("Например, 170") }); else if (nextStep === "purpose") await ctx.reply("Что вы ищете в отношениях?", { reply_markup: force("Напишите коротко о цели") }); else await askTelegram(ctx); return; }
  if (ctx.session.step === "height") { const height = Number(text); if (!Number.isInteger(height) || height < 120 || height > 230) { await ctx.reply("Укажите рост от 120 до 230 сантиметров.", { reply_markup: force("Например, 170") }); return; } d.height = height; begin(ctx, "about"); await ctx.reply("Подсказка: опишите себя от 50 до 1000 слов.\n\n📝 Расскажите о себе", { reply_markup: force("Напишите о себе") }); return; }
...
composer.callbackQuery("profile:create:save", async (ctx) => {
  ...
  const decision = inspectProfileText({ name: d.name, city: d.city, nationality: d.nationality, profession: d.profession, bio: d.bio, purpose: d.purpose });
