...
composer.on("message:text", async (ctx, next) => {
  ...
  const value = ctx.message.text.trim(); const field = ctx.session.editField; const stored = await new DomainStore(ctx).get<Profile>(profileKey(userId(ctx))); if (!stored) { ctx.session.step = "idle"; await ctx.reply("Профиль не найден. Создайте его заново."); return; } const p = withTelegramDefaults(stored); const max = field === "nationality" ? 100 : field === "bio" ? Number.POSITIVE_INFINITY : 500; if (field !== "bio" && (!value || value.length > max)) { await ctx.reply(field === "nationality" ? "Укажите национальность от 1 до 100 символов." : "Значение должно быть от 1 до 500 символов.", { reply_markup: force("Введите новое значение") }); return; } if (field === "telegram") { return; } else if (field === "city") { p.city = value; } else if (field === "age") { const n = Number(value); if (!Number.isInteger(n) || n < 18 || n > 99) { await ctx.reply("Укажите возраст от 18 до 99 лет.", { reply_markup: force("Введите возраст") }); return; } p.age = n; } else if (field === "height") { const n = Number(value); if (!Number.isInteger(n) || n < 120 || n > 230) { await ctx.reply("Укажите рост от 120 до 230 сантиметров.", { reply_markup: force("Например, 170") }); return; } p.height = n; } else (p as unknown as Record<string, unknown>)[field] = value;
