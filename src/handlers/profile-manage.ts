...
composer.on("message:text", async (ctx, next) => {
  ...
  const value = ctx.message.text.trim(); const field = ctx.session.editField; const stored = await new DomainStore(ctx).get<Profile>(profileKey(userId(ctx))); if (!stored) { ctx.session.step = "idle"; await ctx.reply("Профиль не найден. Создайте его заново."); return; } const p = withTelegramDefaults(stored); const max = field === "nationality" ? 100 : field === "bio" ? 10_000 : 500; if (!value || value.length > max) { await ctx.reply(field === "nationality" ? "Укажите национальность от 1 до 100 символов." : field === "bio" ? "Текст слишком длинный. Укажите от 50 до 1000 слов." : "Значение должно быть от 1 до 500 символов.", { reply_markup: force("Введите новое значение") }); return; } if (field === "telegram") { return; } else if (field === "city") { const city = canonicalCity(value); if (!city) { await ctx.reply("📍 Такого города не найдено. Пожалуйста, укажите существующий город", { reply_markup: force("Введите город") }); return; } p.city = city; } else if (field === "age") { const n = Number(value); if (!Number.isInteger(n) || n < 18 || n > 99) { await ctx.reply("Укажите возраст от 18 до 99 лет.", { reply_markup: force("Введите возраст") }); return; } p.age = n; } else if (field === "height") { const n = Number(value); if (!Number.isInteger(n) || n < 120 || n > 230) { await ctx.reply("Укажите рост от 120 до 230 сантиметров.", { reply_markup: force("Например, 170") }); return; } p.height = n; } else (p as unknown as Record<string, unknown>)[field] = value;
  const decision = inspectProfileText({ name: p.name, city: p.city, nationality: p.nationality, profession: p.profession, bio: p.bio, purpose: p.purpose });
  await recordPolicyAudit(ctx, decision.kind === "allowed" ? "allowed" : "allowed-with-suggestion", decision);
  if (decision.kind === "suggestion") { await ctx.reply(decision.message!, { reply_markup: force("Введите чуть подробнее") }); return; }
  if (decision.kind === "explicit") { const event = await enforceViolation(ctx, decision); ctx.session.step = "idle"; await ctx.reply(blockMessage(event!), { reply_markup: menu }); return; }
