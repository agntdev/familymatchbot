...
composer.callbackQuery("profile:edit:telegram:change", async (ctx) => { await ctx.answerCallbackQuery(); ctx.session.step = "telegram_manual"; await ctx.reply("Пожалуйста, укажите ваш Telegram username в формате @username", { reply_markup: force("@username") }); });
